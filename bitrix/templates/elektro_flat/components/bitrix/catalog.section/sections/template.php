	...
	foreach($arResult["SECTIONS"] as $arSection) {?>
		<div class="items-section">
			<div class="items-section__title-wrap<?=($arParams['HIDE_SECTION'] != 'Y' ? ' active' : '')?>">
				...
				<i class="items-section__title-icon fa<?=($arParams['HIDE_SECTION'] != 'Y' ? ' fa-minus' : ' fa-plus')?>"></i>
			</div>
				...
				<div class="catalog-item-cards">
					...
					foreach($arSection["ITEMS"] as $key => $arElement) {
						...
						<div class="catalog-item-card<?=$class?>">
							...
							<?//QUICK_VIEW?>
	                            <?if($inQuickView){?>
	                                <button type="button" id="<?=$arItemIDs['QUICK_VIEW'];?>" class="quick_view"  data-action="view" name="quick_view">
		                                <i class="fa fa-eye"></i>
		                 	            <span> <?=GetMessage("CATALOG_ELEMENT_BTN_QUICK_VIEW")?></span>
		                            </button>
	                             <?}?>
