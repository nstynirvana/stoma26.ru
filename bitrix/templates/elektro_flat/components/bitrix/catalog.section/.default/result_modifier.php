...
foreach ($arResult["ITEMS"] as $key => $arElement) {
    if (!array_key_exists("ZERO-INF", $arElement["PRICE_MATRIX"]["ROWS"])) {
        foreach ($arElement["ITEM_PRICES"] as $keyPrice => $itemPrice) {
            $arResult["ITEMS"][$key]["ITEM_PRICES"][$keyPrice]["MIN_QUANTITY"] = $itemPrice["QUANTITY_FROM"] ? $itemPrice["QUANTITY_FROM"] : 1;
        }
    } else {
        foreach ($arElement["ITEM_PRICES"] as $keyPrice => $itemPrice) {
...
foreach ($arResult["ITEMS"] as $key => $arElement) {
    ...
    if(isset($arPriceMatrix) && is_array($arPriceMatrix))
        $count=0;
    foreach($arPriceMatrix as $key_matrix => $item) {

        foreach($item as $key2 => $item2) {

            $cols = array_keys($arElement["PRICE_MATRIX"]["COLS"]);

            $arPriceMatrix[$key_matrix][$key2]["COLS"] = $arElement["PRICE_MATRIX"]["COLS"][$cols[$count]]["NAME_LANG"];
