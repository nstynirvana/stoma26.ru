		...
		<div class='row'>
			<div class='col-xs-12'>
				...
				if (!$arResult['IS_CASH'] && strlen($arResult['PAYMENT_LINK']))
				{
					...
					<?
					$paySystemService = Bitrix\Sale\PaySystem\Manager::getObjectById($arParams["NEW_PAY_SYSTEM_ID"]);
					if(CSalePdf::isPdfAvailable() && $paySystemService->isAffordPdf()) { ?>
						<p><?=Loc::getMessage("SOPC_PAY_PDF", array("#LINK#" => $arParams["PATH_TO_PAYMENT"]."/?ORDER_ID=".htmlspecialcharsbx($arResult['ORDER_ID'])."&pdf=1&DOWNLOAD=Y"));?></p>
					<?}
				}
				?>
			</div>
