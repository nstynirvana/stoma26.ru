...
if($request->isPost() && check_bitrix_sessid()) {
	...
	switch($action) {
		...
		case "props":
			...
			$arParams = unserialize(base64_decode($signer->unsign($arParams, "sale.products.gift.section")));
			$elementId = $request->getPost("ELEMENT_ID");
			$strMainId = $request->getPost("STR_MAIN_ID");
			
			$offersDef = $request->getPost("OFFERS_DEF");
			if(!empty($offersDef))
				$offersDef = unserialize(base64_decode($signer->unsign($offersDef, "sale.products.gift")));			
			
			$pricesDef = $request->getPost("PRICES_DEF");
			if(!empty($pricesDef))
				$pricesDef = unserialize(base64_decode($signer->unsign($pricesDef, "sale.products.gift")));
