...
$inAdvantages = in_array("ADVANTAGES", $arSetting["CATALOG_DETAIL"]);
$inProductQnt = in_array("PRODUCT_QUANTITY", $arSetting["GENERAL_SETTINGS"]);
$inOffersLinkShow = in_array("OFFERS_LINK_SHOW", $arSetting["GENERAL_SETTINGS"]);
$inBtnBoc = in_array("BUTTON_BOC", $arSetting["CATALOG_DETAIL"]);
$inBtnCheaper = in_array("BUTTON_CHEAPER", $arSetting["CATALOG_DETAIL"]);
$inBtnPayments = in_array("BUTTON_PAYMENTS", $arSetting["CATALOG_DETAIL"]);
$inBtnCredit = in_array("BUTTON_CREDIT", $arSetting["CATALOG_DETAIL"]);
$inBtnDelivery = in_array("BUTTON_DELIVERY", $arSetting["CATALOG_DETAIL"]);
 ...
 */?>
												...
												<?} else {?>
													<div class="add2basket_block">
														<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
															...
															<?}?>
															<button type="button" class="btn_buy detail" name="add2basket"><i class="fa fa-shopping-cart"></i><span><?=($arSetting["NAME_BUTTON_TO_CART"] ? $arSetting["NAME_BUTTON_TO_CART"] : GetMessage("CATALOG_ELEMENT_ADD_TO_CART"))?></span></button>
														</form>
											...
											<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
												<?if(!$arResult["COLLECTION"]["THIS"]) {?>
													<div class="qnt_cont">
														...
														<input type="text" id="quantity_<?=$arItemIDs['ID']?>" name="quantity" class="quantity" value="<?=($arResult['MIN_PRICE']['QUANTITY_FROM']?$arResult['MIN_PRICE']['QUANTITY_FROM']:$arResult['MIN_PRICE']['MIN_QUANTITY'])?>"/>
												...
												<?}?>
												<?if(!$arResult["COLLECTION"]["THIS"]) {?>
													<button type="button" id="<?=$arItemIDs['BTN_BUY']?>" class="btn_buy detail" name="add2basket"><i class="fa fa-shopping-cart"></i><span><?=($arSetting["NAME_BUTTON_TO_CART"] ? $arSetting["NAME_BUTTON_TO_CART"] : GetMessage("CATALOG_ELEMENT_ADD_TO_CART"))?></span></button>
												<?} else {?>
						...
						<div id="<?=$arItemIDs['DELIVERY']?>">
							...
							<?if($arParams["AJAX_OPTION_HISTORY"] == "Y" && $arParams["AJAX_MODE"] == "Y") {
								...
								if($arSetting["USE_GEOLOCATION"] == "Y" && $arSetting["GEOLOCATION_DELIVERY"] == "Y" && !$arResult["COLLECTION"]["THIS"]):
									if(isset($arResult["JS_OFFERS"]) && !empty($arResult["JS_OFFERS"])):
										if($arSetting["OFFERS_VIEW"]["VALUE"] != "LIST"):
											$arOffer = $arResult["JS_OFFERS"][$arResult["OFFERS_SELECTED"]];
				...
				<?foreach($arResult["KIT_ITEMS"] as $key => $arItem) {?>
					<div class="catalog-item-card">
						<div class="catalog-item-info">
							...
							<?//KIT_ITEM_PRICE//?>
							<div class="item-price-cont<?=(!$inOldPrice ? ' one' : '').($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"]) ? ' reference' : '');?>">
								...
								<div class="item-price">
									...
									<?if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])) {?>
										<span class="catalog-item-price-reference">
											<?=CCurrencyLang::CurrencyFormat($arItem["PRICE_DISCOUNT_VALUE"] * $arItem["BASKET_QUANTITY"] * $arSetting["REFERENCE_PRICE_COEF"], $arItem["PRICE_CURRENCY"], true);?>
										</span>
									<?}?>
								</div>
							</div>
...
<?if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"])) {
	$arJSParams = array(
		"CONFIG" => array(
			...
			"USE_GEOLOCATION" => $arSetting["USE_GEOLOCATION"],
			"GEOLOCATION_DELIVERY" => $arSetting["GEOLOCATION_DELIVERY"],
		),
	...
	$arJSParams = array(
		...
		"PRODUCT" => array(
            ...
            "QUANTITY_FROM"=>$arResult['MIN_PRICE']['QUANTITY_FROM'],
		)
	);	
}
