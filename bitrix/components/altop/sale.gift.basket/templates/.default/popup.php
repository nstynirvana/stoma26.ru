...
if($request->isPost() && check_bitrix_sessid()) {
	...
	switch($action) {
		...
		case "props":
			...
			$arParams = unserialize(base64_decode($signer->unsign($arParams, "sale.gift.basket")));			
			$elementId = $request->getPost("ELEMENT_ID");
			$strMainId = $request->getPost("STR_MAIN_ID");
			
			$offersDef = $request->getPost("OFFERS_DEF");
			if(!empty($offersDef))
				$offersDef = unserialize(base64_decode($signer->unsign($offersDef, "sale.gift.basket")));
			
			$pricesDef = $request->getPost("PRICES_DEF");
			if(!empty($pricesDef))
				$pricesDef = unserialize(base64_decode($signer->unsign($pricesDef, "sale.gift.basket")));
